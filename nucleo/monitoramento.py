...
# ══════════════════════════════════════════════════════════════════════════════
# KEY READER — Ctrl+X para parar, Ctrl+C passa normalmente
# ══════════════════════════════════════════════════════════════════════════════
...
def _loop_display(cfg: dict, _stop: threading.Event = None):
    ...
    while True:
        ...
        with _stats_lock:
            seen   = _stats["seen"];  sent  = _stats["sent"]
            erros  = _stats["erros"]; buf   = _stats["buffer"]
            ultimo = _stats["ultimo"]; hb   = _stats["heartbeats"]
            lo     = _stats["login_ok"]; us = _stats["ultimo_status"]
        ...
        if buf > 0:
            spin_txt = f"[ ▶ ]  enviando {buf} eventos..."; spin_cor = C_AV
        elif erros > 0 and us not in ("HTTP 200", "—"):
            spin_txt = f"[ ✗ ]  erro: {us}";               spin_cor = C_ER
        else:
            spin_txt = f"[{_SPIN[idx%4]}]  radar ativo";   spin_cor = C_NE
        ...
        left = [
            ...
            sep('╠','╣'),                                   # 18
            box(f"Uptime    : {uptime_s}",         C_D),   # 19
            box(f"Threat    : {threat}",           t_cor), # 20
            sep('╠','╣'),                                   # 21
            box('eventos/lote',                    C_D),   # 22
            row(f"║  {hist_s}║",                  C_OK),  # 23
            sep('╠','╣'),                                   # 24
            box('Ctrl+X para parar',               C_D),   # 25
        ]
        ...
        if _RADAR_OK and radar_visivel:
            ...
            right = [
                C_D + '[ janela pequena ]' + _R,
                C_D + (f'aumente {dc} colunas' if dc else 'redimensione') + _R,
            ]
            try: _radar._tick()
            except Exception: pass
        else:
    ...
    try:
        ...
        if resp.status_code == 403:
            try:
                dados = resp.json()
                if "token" in dados:
                    cfg["token"] = dados["token"]; salvar_config(cfg)
            except Exception: pass
            return False
        ...
        try:
            ...
            if novo and novo != cfg.get("token", ""):
                cfg["token"] = novo; salvar_config(cfg)
        except Exception: pass
        return True
...
def modo_auto(cfg: dict):
    ...
    def _hb_log():
        while True:
            time.sleep(60)
            with _stats_lock:
                s  = _stats["seen"];   e  = _stats["sent"]
                er = _stats["erros"];  hb = _stats["heartbeats"]
                lo = _stats["login_ok"]; st = _stats["ultimo_status"]
            print(f"[{agora()}] stats | vistos={s} | enviados={e} | erros={er} | "
                  f"heartbeats={hb} | login={'ok' if lo else 'falhou'} | http={st}",
                  flush=True)
